...
import bgImg from "../assets/spaceshootebg2.jpg";
...
class PlayGame extends Phaser.Scene {
  init(data) {
    ...
    this.walletaddress = data.walletADD;
    console.log('playDATA.walletadd', this.walletaddress);
  }
  preload() {
    //the sprites we need
  ...
  create() {
    ...
    let bg = this.add.image(0, 0 ,'bgimg');
    bg.setScale(3);
  ...
  */
  check_for_winner = (score) => {
    if (score >= 10) {
      let players = [{ name: this.name, score: this.score , wallet: this.walletaddress}];
      for (let other in this.others) {
        players.push({
          name: this.others[other].name,
          score: this.others[other].score,
          wallet: this.others[other].wallet
        });
      }
